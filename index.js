...
const handleAIRequest = async (req, res, body) => {
	try {
		...
		const aiResponse = await axios.post(`${aiServiceUrl}/process-text`, { text: body.text });

		console.log("AI service responded with:", aiResponse.data);
		res.writeHead(200, { "Content-Type": "application/json" });
		res.end(JSON.stringify({ summary: aiResponse.data.summary }));
	} catch (error) {
		console.error("Error calling AI service:", error.message, error.response ? error.response.data : error);
		res.writeHead(500, { "Content-Type": "application/json" });
